...
public class FileLog implements FileHandler {
	...
	public FileLog(DrawFrame frame, DrawModel model, DrawController controller) {
		this.frame = frame;
		this.model = model; 
		...
		try {
			...
			switch(commands1[0]) {
				case "Added":
					...
					controller.executeCommand(new CommandAddShape(shape, model));
				...
				case "Updated":
					...
					if (oldShape instanceof Point) {
						...
						controller.executeCommand(
							new CmdUpdateCircle((Circle) model.getByIndex(index), newCircle)
						);
						...
						controller.executeCommand(
							new CmdUpdateHexagon((HexagonAdapter) model.getByIndex(index), newHexagon)
						);
						frame.getList().addElement("Updated->" + oldShape.toString() + "->" + newHexagon.toString());
					}
					break;
				case "Deleted":
					Shape deletedShape = parseShape(commands1[1].split(":")[0],
							commands1[1].split(":")[1]);
					controller.executeCommand(new CommandRemoveShape(deletedShape, model));
					frame.getList().addElement("Deleted->" + deletedShape.toString());
					controller.handleSelect("Deleted", "parser");
					break;
				case "Moved to front":
					Shape shapeMovedToFront = parseShape(commands1[1].split(":")[0],
							commands1[1].split(":")[1]);
					...
					break;
				case "Moved to back":
					Shape shapeMovedToBack = parseShape(commands1[1].split(":")[0],
							commands1[1].split(":")[1]);
				...
				case "Bringed to front":
					Shape shapeBringedToFront = parseShape(commands1[1].split(":")[0],
							commands1[1].split(":")[1]);
					controller.executeCommand(new CommandBringToFront(model, shapeBringedToFront,
							model.getAll().size() - 1));
					frame.getList().addElement("Bringed to front->" + shapeBringedToFront.toString());
					break;
				case "Bringed to back":
					Shape shapeBringedToBack = parseShape(commands1[1].split(":")[0],
							commands1[1].split(":")[1]);
					...
					break;
				case "Selected":
					Shape selectedShape = parseShape(commands1[1].split(":")[0],
							commands1[1].split(":")[1]);
					int index1 = model.getIndexOf(selectedShape);
					controller.executeCommand(new CommandSelectShape(model.getByIndex(index1), true));
					frame.getList().addElement("Selected->" + selectedShape.toString());
					controller.handleSelect("Selected", "parser");
					break;
				case "Unselected":
					Shape unselectedShape = parseShape(commands1[1].split(":")[0],
							commands1[1].split(":")[1]);
					int index2 = model.getIndexOf(unselectedShape);
					controller.executeCommand(new CommandSelectShape(model.getByIndex(index2), false));
